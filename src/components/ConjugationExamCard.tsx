...
import type { ConjugationExamQuestion } from "@/lib/conjugationExam";

type ConjugationExamCardProps = {
  question: ConjugationExamQuestion;
...
export function ConjugationExamCard({
  question,
  currentQuestion,
  totalQuestions,
  isLastQuestion,
  ...
  onAnswer,
  onNext,
}: ConjugationExamCardProps) {
  const answered = Boolean(selectedAnswer);
  const isCorrect = selectedAnswer === question.answer;
  const progressPercent = totalQuestions === 0 ? 0 : (currentQuestion / totalQuestions) * 100;
  const speechText = question.example.exampleJa ?? question.example.result;
    ...
    <section className="rounded-lg border border-white/80 bg-white/90 p-4 shadow-card sm:p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <span className="rounded-full bg-sakura/25 px-3 py-1 text-xs font-bold text-ink">
          {question.type === "result-choice" ? "Result Choice" : "Form Choice"}
        </span>
